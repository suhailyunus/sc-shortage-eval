...
\textbf{Proxy labels and target variable construction.} \citet{obermeyer2019dissecting} show that a widely deployed healthcare algorithm, trained to predict health-care \emph{cost} as a proxy for health \emph{need}, encoded substantial racial bias because the two constructs diverge systematically -- patients with equal need but unequal historical spending received unequal predicted risk. \citet{guerdan2025measurement} interview practicing data scientists and find that target variable construction for an unobservable construct is routinely a negotiated, iterative process balancing validity against predictability, simplicity, and resource constraints, rather than a single well-specified choice. The store-volume confound in Section~\ref{sec:confound} is a concrete instance of the general failure mode both papers describe: a proxy (a pooled percentile threshold) that is predictable and stable under standard evaluation, yet systematically diverges from the construct it was meant to represent (demand unusual for a specific location, not aggregate sales volume).

\textbf{Weak supervision and label noise.} \citet{ratner2017snorkel} formalize the broader practice of constructing training labels from heuristic, potentially correlated, and imperfect sources rather than exhaustive manual annotation, and provide tooling to model and denoise the resulting label uncertainty explicitly. \citet{frenay2014classification} survey the consequences of label noise for classifier training and evaluation, including the risk that a systematic (non-random) noise process -- as opposed to independent, random mislabeling -- can bias a model's learned decision boundary in a specific, predictable direction. Our stress label is not weakly supervised in the technical sense of combining multiple heuristic labeling functions, but the item-only threshold's systematic, volume-correlated error (Section~\ref{sec:confound}) is exactly the kind of structured, non-random label noise this literature treats as more consequential than i.i.d.\ label flipping.

\textbf{Demand classification.} \citet{syntetos2005categorization} propose classifying demand series by average inter-demand interval and the squared coefficient of variation of demand sizes, yielding the smooth/erratic/intermittent/lumpy taxonomy we use to design a stratified sample (Section~\ref{sec:scale}) that deliberately oversamples the harder, more business-relevant intermittent and lumpy segments rather than sampling uniformly at random. \citet{hyndman2011optimal} address a related but distinct problem in retail and tourism demand: reconciling independently produced forecasts across a hierarchy (item, store, region) so that aggregated and disaggregated forecasts remain mutually consistent. We do not perform hierarchical reconciliation -- each item-store series is modeled independently -- but the store-volume confound in Section~\ref{sec:confound} is itself evidence that a modeling decision made at one level of the M5 hierarchy (a threshold pooled across the store level) can silently reintroduce store-level structure into what is intended to be an item-store-level label, the same cross-level leakage concern that motivates reconciliation methods in the forecasting literature. \citet{liu2025machine} apply classical machine learning directly to inventory stockout prediction on a large-scale (1.6 million SKU), severely imbalanced retail dataset, and report that near-term demand signal and current inventory level are the dominant predictors; our work addresses a harder-to-observe adjacent problem -- M5 contains no inventory field, so stockout itself cannot be the label -- but shares the same class-imbalance and large-scale evaluation concerns their study raises.
...
\textbf{Probability calibration.} \citet{niculescu2005predicting} show that boosted trees systematically distort predicted probabilities away from 0 and 1 in a characteristic sigmoid-shaped pattern, and evaluate Platt scaling \citep{platt1999probabilistic} and isotonic regression \citep{zadrozny2002transforming} as corrections. \citet{wallace2012class} address the specific interaction we rely on throughout Section~\ref{sec:calibration}: they show that class-imbalance correction techniques (including the cost-reweighting approach used here via \texttt{scale\_pos\_weight}) systematically bias the resulting probability estimates, and that this bias is a separate problem from the ranking-quality benefit such techniques are adopted for. Our finding that a boosted-tree model trained with class-imbalance reweighting is severely miscalibrated, yet retains good ranking quality, is a direct empirical instance of the ranking/calibration separation both bodies of literature predict; our contribution is showing, on a real cost-sensitive decision problem, that the correction changes what a score \emph{means} far more than it changes which decision the score implies.
...
\begin{equation}
\text{stress\_event}_{i,s,t} = \mathbf{1}\left\{\text{sales}_{i,s,t} > Q_{0.90}\big(\text{sales}_{i,s,\cdot} \mid t' \le t_{\text{split}}\big)\right\}
\end{equation}
...
We computed each store's holdout stress rate and average daily sales. Because ten aggregated observations provide limited precision, we report a $p$-value and Fisher interval and supplement the store-level result with a within-item analysis over all 30{,}490 item-store series: for each series we compute the item-level mean of both mean daily sales and the stress rate, subtract that item-level mean from the series' own value (demeaning both variables within item), and Pearson-correlate the resulting residuals. This removes any correlation attributable to differences between items and isolates the association specifically attributable to differences between stores carrying the same item.
...
Platt scaling is strictly monotone. Isotonic regression is only \emph{non-decreasing}: it can collapse groups of distinct raw scores into tied plateaus, so a single calibrated threshold is not in general guaranteed to reproduce the exact alert set that a raw threshold at the same nominal rank position would select. This is a real possibility, not a technicality -- tied plateaus are precisely why the retrospective oracle search below lands on different alert counts for the two score scales.

At full scale, we verified the ex-ante case directly rather than assuming the general guarantee applies: the raw threshold selected on the calibration period was 0.9041, producing \$28{,}806 on final evaluation (6{,}922 alerts; 5{,}085 true positives; 1{,}837 false positives). The corresponding isotonic threshold, 0.5498, was applied independently to the isotonic-calibrated final-evaluation scores and produced the identical alert count, true-positive count, false-positive count, and \$28{,}806 net. In this specific run, no tied plateau happened to fall across the decision boundary, so the two policies coincided exactly -- but this is an empirical outcome of this particular score distribution, not a property guaranteed by isotonic calibration in general, and a different dataset or threshold could see the two policies diverge. The two thresholds' \emph{retrospective} final-period oracle values are not identical (\$30{,}506 raw versus \$30{,}070 isotonic), because the oracle search independently re-optimizes the threshold on final-evaluation data for each score scale, and isotonic's tied plateaus mean the two searches are not guaranteed to land on matching rank positions -- this divergence is the concrete manifestation of the general non-guarantee. With heterogeneous case costs or decisions that consume probabilities directly, calibration can still affect the action itself; it did not in this single-threshold, constant-cost ex-ante comparison specifically, but that should be read as an empirical finding for this case, not a general property of isotonic calibration.
...
We compare XGBoost against LightGBM \citep{ke2017lightgbm}, CatBoost \citep{prokhorenkova2018catboost}, and a logistic regression baseline, at the 1{,}000-series scale. Following Section~\ref{sec:calibration}'s convention, Brier Skill Score is reported on isotonic-calibrated scores, since raw class-weighted scores from every boosted-tree model here have deeply negative raw-score BSS despite good ranking (Average Precision) -- an expected consequence of severe raw overconfidence, not a defect specific to any one library, and the same phenomenon already implied by Table~\ref{tab:calibration}'s own raw-versus-reference Brier gap. Both the ex-ante and oracle threshold searches use the same exact, tied-group-endpoint algorithm as Sections~\ref{sec:scale} and \ref{sec:calibration}, which explicitly includes a no-alert candidate; an earlier version of this script used a coarse fixed grid without that safeguard and could report an oracle value below \$0, which is not attainable since flagging nothing is always an available policy.

\begin{table}[h]
\centering
\caption{Model-family comparison, 1{,}000-series scale, corrected label, mean [range] across three algorithmic-randomness seeds on this fixed sample (see caveat below). Brier Skill Score (BSS) is reported after isotonic calibration, matching Section~\ref{sec:calibration}'s convention.}
\label{tab:modelrobust}
\small
\begin{tabular}{lrrrr}
\toprule
Model & AP (raw scores) & BSS (calibrated) & Ex-ante net & Oracle net \\
\midrule
XGBoost & 0.179 [0.166, 0.186] & 0.036 [0.028, 0.039] & $+\$23$ [0, 70] & $+\$41$ [0, 104] \\
CatBoost & 0.182 [0.182, 0.183] & 0.036 [0.036, 0.037] & $-\$10$ [$-54$, 16] & $+\$38$ [34, 46] \\
LightGBM & 0.150 [0.147, 0.153] & 0.024 [0.022, 0.025] & \$0 [0, 0] & \$0 [0, 0] \\
Logistic regression & 0.134 [fixed] & 0.012 [fixed] & \$0 [fixed] & \$0 [fixed] \\
\bottomrule
\end{tabular}
\end{table}

Two findings are stable across all three seeds. First, XGBoost and CatBoost are higher than both LightGBM and the logistic regression baseline on average precision and calibrated Brier Skill Score in these three fixed-sample runs; the gap between XGBoost and CatBoost themselves, by contrast, is small enough -- and their ranges overlap closely enough -- that the two are not meaningfully distinguishable at this scale. Three algorithmic-randomness seeds on a single fixed sample do not establish statistical superiority of one over the other; both produced higher AP and calibrated BSS than LightGBM and logistic regression in these fixed-sample runs, which is the extent of what this experiment establishes. Second, the economic outcome is close to zero for every model, including logistic regression once its no-alert floor is correctly enforced by the threshold-search algorithm described above -- consistent with the near-breakeven business case already established in Section~\ref{sec:scale} at this sample size. Logistic regression's clearly weaker ranking (lowest AP and BSS of all four models) does not translate into a distinguishable economic loss here, since the near-breakeven regime means most models converge on ``flag little or nothing'' as the best available policy regardless of ranking quality. Logistic regression's metrics do not vary across seeds because the \texttt{lbfgs} solver used here is deterministic given fixed data; \texttt{random\_state} affects only the three boosted-tree models' bagging behavior.

\textbf{What the three seeds do and do not establish.} These three seeds vary each boosted-tree model's own internal randomness (row and column subsampling) on a single, fixed 100-item sample -- they measure algorithmic-randomness variance only, not uncertainty from item selection, from a different chronological window, or from resampling the underlying series (which is what the independent 3-seed \emph{item-sampling} check in Section~\ref{sec:scale} measures at the 5{,}000-series scale). The findings above should be read as consistent across three algorithmic-randomness seeds on this fixed sample, not as a broader claim about sampling variance.

\textbf{A data-reuse caveat.} The calibration period in this comparison serves three roles at once: early-stopping validation during model fitting, isotonic calibrator fitting, and ex-ante threshold selection. A four-way split with separate periods for each role would remove any risk of these roles interacting; we did not implement one here for compute-budget reasons, the same limitation already noted for Section~\ref{sec:calibration} below. This comparison is informative about relative model ranking; it is not a fully independent confirmation free of any shared-data risk.
...
Reproduction uses Python 3.12.3, NumPy 2.4.4, scikit-learn 1.8.0, and XGBoost 3.4.1. The Section~\ref{sec:calibration} full-catalog results were generated with pandas 2.3.3; the Section~\ref{sec:modelrobustness} model-family comparison additionally uses LightGBM 4.7.0 and CatBoost 1.2.10, and was verified to produce equivalent results (AP within 0.001) under both pandas 2.3.3 and pandas 3.0.2, run back-to-back as a direct check rather than assumed. The commands and machine-readable outputs are in:
\begin{itemize}[nosep,leftmargin=*]
    \item \path{scripts/paper_audit.py}
    \item \path{scripts/train_calibrate_full_catalog.py}
    \item \path{scripts/model_comparison_robust.py}
    \item \path{paper_draft/generated/}
\end{itemize}
Code is available at \url{https://github.com/suhailyunus/supply-chain-stress-prediction}; an immutable release should be archived for submission.

\section{Conclusion}

This case study shows that a proxy label can be predictable and still be misaligned with its intended meaning. Testing grouping choices exposed a strong volume association, temporal regression tests exposed two leakage mechanisms, and a proper prevalence reference tempered an initially overstated calibration claim. Isotonic calibration repaired overconfidence but added only modest Brier skill over prevalence. Under one global threshold and constant costs, calibration changed probability meaning far more than decision value, while a model-family robustness check supports the conclusion that these findings hold across gradient-boosting libraries. The broader lesson is procedural: validate the target estimand, the information set, probability meaning, and operating policy as separate objects.
...
\bibitem[Fr\'enay and Verleysen(2014)]{frenay2014classification}
Fr\'enay, B. and Verleysen, M. (2014).
...
\bibitem[Guerdan et al.(2025)]{guerdan2025measurement}
Guerdan, L., Saxena, D., Chancellor, S., Wu, Z.~S., and Holstein, K. (2025).
\newblock Measurement as bricolage: Examining how data scientists construct target variables for predictive modeling tasks.
\newblock \emph{Proceedings of the ACM on Human-Computer Interaction}, 9(CSCW).

\bibitem[Hyndman et al.(2011)]{hyndman2011optimal}
Hyndman, R.~J., Ahmed, R.~A., Athanasopoulos, G., and Shang, H.~L. (2011).
\newblock Optimal combination forecasts for hierarchical time series.
\newblock \emph{Computational Statistics \& Data Analysis}, 55(9):2579--2589.
...
\bibitem[Liu et al.(2025)]{liu2025machine}
Liu, Y., Kalaitzi, D., Wang, M., and Papanagnou, C. (2025).
\newblock A machine learning approach to inventory stockout prediction.
\newblock \emph{Journal of Digital Economy}, 4:144--155.
...
\newblock \emph{Science}, 366(6464):447--453.
...
\newblock \emph{Proceedings of the VLDB Endowment}, 11(3):269--282.
...
\bibitem[Wallace and Dahabreh(2012)]{wallace2012class}
Wallace, B.~C. and Dahabreh, I.~J. (2012).
\newblock Class probability estimates are unreliable for imbalanced data (and how to fix them).
\newblock In \emph{Proceedings of the 12th IEEE International Conference on Data Mining}, pages 695--704.
